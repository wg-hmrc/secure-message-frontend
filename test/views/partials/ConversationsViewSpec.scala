...
import views.html.partials.{ archivedMessages, conversations, recentMessages }
    ...
    "have all mandatory information" in new TestClass {
      ...
      val recentConversations = Seq(
        Conversation(
          ...
          "12 January 2021"))
      val archieveConversations = Seq(
        Conversation(
          "222",
          "/secure-messaging/conversation-messages/222",
          "HMRC exports",
          "MRN 30GB16046891253600",
          "13 January 2020"))

      val recentMessagesPartial = recentMessagesHtml.render(recentConversations, request, messages)
      val archieveMessagesPartial = archiveMessagesHtml.render(archieveConversations, request, messages)

      val htmlContent = conversationsTemplate
        .render(recentMessagesPartial, archieveMessagesPartial, request, messages)
        .toString
      ...
      htmlContent must include(
        "<a href=/secure-messaging/conversation-messages/222 class=\"govuk-link\" >MRN 30GB16046891253600</a>        needs action")
