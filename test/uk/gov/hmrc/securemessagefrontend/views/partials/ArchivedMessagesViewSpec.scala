...
class ArchivedMessagesViewSpec extends PlaySpec with GuiceOneAppPerSuite{
  "Archived messages template" must {
    "have all mandatory information" in new TestClass {
      val conversations = Seq(Conversation("111", "/secure-messaging/conversation-messages/111",  "HMRC exports", "MRN 20GB16046891253600", "12 January 2021"))

      val htmlString =  archivedMessagesHtml.render(conversations, request, messages, appConfig).toString
      ...
      htmlString must include("<a href=/secure-messaging/conversation-messages/111 class=\"govuk-link\" >MRN 20GB16046891253600</a>        needs action")
